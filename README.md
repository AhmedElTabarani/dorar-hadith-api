...
> يفضل عمل `fork` او `clone` للمشروع ورفعه على استضافة او سيرفر خاص بك واستخدام رابك الـ `API` الخاص بك، لان الرابط التالي استضافة مجانية وليس ثابتا، سأحاول إن شاء الله رفعه على استضافة مدفوعة بـ `domain` خاص ثابت

- الرابط: https://dorar-hadith-api.cyclic.cloud/
- يتم عمل `cache` لكل عملية بحث لمدة `5` ثواني
- هناك حد للاستخدام: `300` عملية بحث في اليوم لكل `IP` 
...
### رفع المشروع على سيرفر خاص  

يمكنك عمل `fork` أو `clone` لهذا المشروع ورفعه على سيرفر خاص بك الخاص إن اردت **وهذا ما أنصح به** لكي تتجنب أي مشاكل
