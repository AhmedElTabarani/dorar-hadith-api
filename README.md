...
> يفضل عمل `folk` او `clone` للمشروع ورفعه على إستضافة او سيرفر خاص بك واستخدام رابك الـ `API` الخاص بك، لان الرابط التالي استضافة مجانية وليس ثابتا، سأحاول إن شاء الله رفعه على إستضافة مدفوعة بـ `domain` خاص ثابت
...
يمكنك عمل `folk` أو `clone` لهذا المشروع ورفعه على سيرفر خاص بك الخاص إن اردت  
